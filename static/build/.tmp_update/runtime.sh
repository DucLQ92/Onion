...
launch_game() {
    ...
    if [ $is_game -eq 0 ] || [ -f "$rompath" ]; then
        if [ "$romext" == "miyoocmd" ]; then
            ...
            if [ $is_game -eq 1 ] && [ ! -f /tmp/.offOrder ] && [ -f /tmp/.displaySavingMessage ]; then
                ...
                infoPanel --title " " --message "Đang lưu..." --persistent --no-footer &
    ...
    if [ $retval -eq 404 ]; then
        infoPanel --title "Không tìm thấy tập tin" --message "Không tìm thấy tập tin được yêu cầu." --auto
    elif [ $retval -ge 128 ] && [ $retval -ne 143 ] && [ $retval -ne 255 ]; then
        infoPanel --title "Đã xảy ra lỗi nghiêm trọng" --message "Chương trình đột ngột dừng.\n(Mã lỗi: $retval)" --auto
    fi
